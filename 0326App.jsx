...
/**
 * SAT THỦ KHOA DESIGN SYSTEM & LOGIC - FINAL VERSION (Fixed Speed Review)
 * * Colors:
...
const QUOTES = [
  ...
  { text: "Thiên tài chỉ là 1% cảm hứng, 99% là mồ hôi.", author: "Thomas Edison" },
  { text: "Sự học như con thuyền đi trên dòng nước ngược, không tiến ắt phải lùi.", author: "Cổ nhân" },
  { text: "Don't let what you cannot do interfere với những gì bạn có thể làm.", author: "John Wooden" }
];
...
export default function App() {
  ...
  // Nền tảng tự động cung cấp Runtime Key cho các Shared Link, không cần config rườm rà.
  const apiKey = ""; 
  ...
  const getLeaderboard = () => {
    const currentUser = { name: "Bạn", xp: xp, id: "me", isUser: true };
  ...
  const updateWordsToday = () => {
    ...
    if (newCount === DAILY_GOAL) {
      ...
      showNotification(`🎉 Hoàn thành mục tiêu ngày! +${XP_REWARDS.DAILY_GOAL_MET} XP`, 'success');
    }
  };
  ...
  const exportToExcel = () => {
    if (history.length === 0) {
      showNotification("Bạn chưa có từ vựng nào trong lịch sử để xuất!", "info");
      return;
    }
    ...
    const rows = history.map((item, index) => {
      ...
      return [
        ...
        clean(item.origin || "Đang cập nhật"),
  ...
  const startSpeedReview = () => {
    ...
    if (validHistory.length < 4) {
      showNotification("Cần ít nhất 4 từ vựng đầy đủ để mở khóa chế độ này!", "info");
      return;
    }
  ...
  const generateAIContent = async (word) => {
    const systemPrompt = `Bạn là chuyên gia luyện thi SAT (SAT Thủ Khoa).
YÊU CẦU: Trả về JSON thuần.
1. "origin": Etymology giống Oxford Dictionary.
2. "suggestions": 3 từ SAT nâng cao liên quan. QUAN TRỌNG: CHỈ TRẢ VỀ TỪ TIẾNG ANH (Ví dụ: "Serendipity").
3. "quiz.explanation": Giải thích tại sao đáp án đúng và tại sao các đáp án khác sai.
4. "quizzes": Mảng 3 câu hỏi (Dễ -> Trung Bình -> Khó).
5. "synonyms": Mảng từ đồng nghĩa.
6. "antonyms": Mảng từ trái nghĩa.

JSON Structure:
{
  "word": "từ vựng được hỏi", "phonetic": "/.../", "type": "adj.", "vietnamese": "...", "origin": "...",
  "blurb": "...", "synonyms": ["..."], "antonyms": ["..."],
  "examples": [{"type": "Classic Literature", "source": "...", "text": "..."}],
  "quizzes": [
     { "level": "Dễ", "question": "...", "options": ["A", "B", "C", "D"], "answer": "...", "explanation": "..." },
     { "level": "Trung bình", "question": "...", "options": ["A", "B", "C", "D"], "answer": "...", "explanation": "..." },
  ...
  "suggestions": ["...", "...", "..."]
    ...
    const payload = {
      contents: [{ parts: [{ text: `Hãy giải thích từ vựng: "${word}"` }] }],
  ...
  const handleSearch = async (e, wordOverride = null) => {
    ...
    if (aiResult) {
      ...
      setResult({
        word: searchTerm, phonetic: "Error", type: "N/A", vietnamese: "Lỗi kết nối", origin: "...",
        blurb: "Đã có lỗi kết nối với máy chủ AI. Vui lòng thử lại sau vài giây.", synonyms: [], antonyms: [], examples: [], quizzes: [], suggestions: []
      });
  ...
  const handleQuizAnswer = (option) => {
    ...
    if (selected === correct) {
      ...
      showNotification("Chưa chính xác, hãy đọc giải thích nhé!", "info");
    }
  };
    ...
    <>
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 relative overflow-hidden mb-6">
          ...
          <div className="relative z-10">
            <div className="flex items-center mb-4">
              ...
              <div>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Cấp Độ</p>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
               <p className="text-xs text-slate-400 font-bold uppercase mb-2">Hoạt động tuần này</p>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 mb-6">
          <div className="flex justify-between items-center mb-4">
             <h3 className="font-bold text-[#004488] uppercase flex items-center"><Trophy className="w-5 h-5 mr-2 text-gold" /> Bảng Phong Thần</h3>
             <span className="text-xs text-slate-400 font-bold bg-slate-100 px-2 py-1 rounded-full">Tuần này</span>
          </div>
              ...
              {getLeaderboard().map((u, idx) => (
                  <div key={u.id} className={`flex items-center justify-between p-3 rounded-xl transition hover:scale-[1.02] ${u.isUser ? 'bg-blue-50 border border-blue-200 shadow-sm' : 'bg-slate-50'}`}>
                      <div className="flex items-center">
                          ...
                          <span className={`font-bold ${u.isUser ? 'text-[#004488]' : 'text-slate-700'}`}>{u.name} {u.isUser && "(Bạn)"}</span>
                      </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg border-t-4 border-green-500 relative overflow-hidden mb-6">
          <div className="flex justify-between items-center mb-2">
            <h3 className="font-bold text-slate-700 uppercase flex items-center"><Target className="w-5 h-5 mr-2 text-green-600" /> Mục tiêu ngày</h3>
            <span className="text-xs font-bold text-slate-500">{wordsToday}/{DAILY_GOAL} từ</span>
          </div>
          ...
          {wordsToday >= DAILY_GOAL && (
            <div className="text-center bg-green-50 text-green-700 p-2 rounded-lg font-bold text-sm animate-pulse">🎉 Xuất sắc! +{XP_REWARDS.DAILY_GOAL_MET} XP</div>
          )}
      </div>
    </>
  );
  ...
  const renderSearchSection = () => (
    <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 mb-6">
      <h2 className="text-lg font-bold text-[#004488] mb-4 uppercase flex items-center justify-between">
        <span className="flex items-center"><Search className="w-5 h-5 mr-2 text-amber-500" /> Tra từ thông minh</span>
      </h2>
      <form onSubmit={(e) => handleSearch(e)} className="space-y-4">
        <input type="text" value={inputWord} onChange={(e) => setInputWord(e.target.value)} placeholder="Nhập từ vựng..." className="w-full px-4 py-3 text-lg border-2 border-slate-200 rounded-xl focus:border-[#004488] outline-none transition font-medium" />
        <button type="submit" disabled={loading} className={`w-full py-3 px-6 bg-amber-400 hover:bg-amber-300 text-[#004488] font-bold text-lg uppercase rounded-xl shadow-[0_4px_0_rgb(180,83,9)] hover:translate-y-[2px] transition-all flex justify-center items-center ${loading ? 'opacity-70' : ''}`}>
          {loading ? <RefreshCw className="animate-spin w-6 h-6" /> : <><Sparkles className="w-5 h-5 mr-2" /> Phân Tích AI (+15 XP)</>}
        </button>
      </form>
    </div>
  );
  ...
  const renderHistorySection = () => (
    <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 mb-6">
        <div className="flex justify-between items-center mb-4">
            <h3 className="text-sm font-bold text-slate-400 uppercase flex items-center"><History className="w-4 h-4 mr-1" /> Kho từ đã học</h3>
            <div className="flex space-x-1">
              <button onClick={startSpeedReview} className="p-2 bg-orange-50 text-orange-600 rounded hover:bg-orange-100 transition flex items-center space-x-1" title="Thử thách tốc độ"><Timer className="w-4 h-4" /> <span className="text-xs font-bold hidden sm:inline">Tốc độ</span></button>
              <button onClick={() => { setCurrentCardIndex(0); setIsFlipped(false); setShowFlashcards(true); }} className="p-2 bg-blue-50 text-[#004488] rounded hover:bg-blue-100 transition flex items-center space-x-1" title="Ôn tập Flashcard"><Layers className="w-4 h-4" /> <span className="text-xs font-bold hidden sm:inline">Flashcard</span></button>
              <button onClick={exportToExcel} className="p-2 bg-green-50 text-green-600 rounded hover:bg-green-100 transition flex items-center space-x-1" title="Xuất Excel"><FileSpreadsheet className="w-4 h-4" /> <span className="text-xs font-bold hidden sm:inline">Excel</span></button>
            </div>
        </div>
        <div className="flex flex-wrap gap-2 max-h-60 overflow-y-auto">
          ...
          )) : <p className="text-sm text-slate-400 italic">Chưa có lịch sử. Hãy tra từ để làm giàu kho từ vựng!</p>}
        </div>
    </div>
  );
    ...
    <div ref={resultRef}>
      {!result && !loading && (
        <div className="h-full flex flex-col items-center justify-center py-6 lg:py-16 space-y-8 animate-fade-in-up">
          ...
          <div className="w-full max-w-2xl text-center px-6">
             ...
             <p className="text-sm font-bold text-[#004488] mt-2 uppercase tracking-widest">— {dailyQuote.author}</p>
          </div>
          ...
          {reviewSuggestion && (
             <div className="w-full max-w-md bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-xl border border-blue-100 flex items-center justify-between cursor-pointer hover:shadow-md transition" onClick={() => handleSearch(null, reviewSuggestion.word)}>
                <div className="flex items-center">
                   ...
                   <div>
                      <p className="text-xs text-slate-400 uppercase font-bold">Góc ôn tập</p>
          ...
          <div className="w-full max-w-md bg-white p-6 rounded-2xl shadow-xl border-t-4 border-orange-400 transform transition hover:scale-105 duration-300 relative group cursor-pointer" onClick={() => handleSearch(null, dailyWord)}>
              ...
              <div className="flex justify-between items-center mb-4">
                 <h3 className="font-bold text-[#004488] uppercase flex items-center"><Calendar className="w-5 h-5 mr-2 text-orange-500" /> Từ của hôm nay</h3>
              ...
              <div className="text-center py-4">
                 ...
                 <span className="text-sm text-slate-400 italic">Nhấn để học ngay</span>
              </div>
          </div>
      ...
      {loading && (
        <div className="space-y-6">
            ...
            <div className="text-center text-slate-400 animate-bounce text-sm">AI đang chuẩn bị nội dung chuẩn "Thủ Khoa"...</div>
        </div>
      )}
        ...
        <div className="space-y-8 animate-fade-in-up pb-24 lg:pb-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl shadow-md border-l-8 border-slate-400 p-6">
                  <div className="flex items-center mb-4"><BookOpen className="w-6 h-6 text-slate-600 mr-2" /><h3 className="text-lg font-bold text-[#004488] uppercase">Gốc từ (Origin)</h3></div>
                  <p className="text-lg text-slate-700 font-serif leading-relaxed italic">{result.origin || "Đang cập nhật..."}</p>
              </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                  <h4 className="font-bold text-[#004488] uppercase text-sm mb-3 flex items-center">
                    <span className="w-2 h-2 rounded-full bg-green-500 mr-2"></span> Từ Đồng Nghĩa (Synonyms)
                  </h4>
               ...
               <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                  <h4 className="font-bold text-[#004488] uppercase text-sm mb-3 flex items-center">
                    <span className="w-2 h-2 rounded-full bg-red-500 mr-2"></span> Từ Trái Nghĩa (Antonyms)
                  </h4>
                  <div className="flex flex-wrap gap-2">
          ...
          {result.examples && result.examples.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-6 md:p-8">
              <h3 className="text-xl font-bold text-[#004488] uppercase mb-4">Ngữ cảnh & Văn học</h3>
          ...
          {(result.quizzes || result.quiz) && (
            <div className="bg-slate-900 rounded-2xl shadow-xl p-6 md:p-8 text-white relative overflow-hidden transition-all duration-300">
              <div className="flex justify-between items-center mb-6 relative z-10">
                <h3 className="text-xl font-bold text-[#FFD700] uppercase flex items-center"><Brain className="w-6 h-6 mr-2" /> Tháp Kiến Thức</h3>
                <button onClick={() => { setQuizMode(!quizMode); setCurrentQuizIndex(0); setQuizStatus({}); }} className="text-xs font-bold bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full transition">{quizMode ? "Ẩn" : "Hiện"}</button>
              </div>
              ...
              {(() => {
                  ...
                  return !quizMode ? (
                    <div className="text-center py-4">
                      <p className="text-slate-300 mb-4">Chinh phục 3 cấp độ để nhận tối đa XP!</p>
                      <button onClick={() => setQuizMode(true)} className="px-8 py-3 bg-[#004488] hover:bg-blue-700 text-white font-bold rounded-xl border border-blue-500 transition shadow-lg">Bắt đầu Quiz</button>
                    </div>
                  ) : (
                    <div className="animate-fade-in relative z-10">
                      <div className="flex justify-between items-center mb-4 text-sm font-bold text-slate-400">
                         <span className={`px-2 py-1 rounded ${currentQuizIndex === 0 ? 'bg-green-900 text-green-300' : currentQuizIndex === 1 ? 'bg-yellow-900 text-yellow-300' : 'bg-red-900 text-red-300'}`}>
                           Level {currentQuizIndex + 1}: {currentQuiz.level || (currentQuizIndex === 0 ? 'Dễ' : currentQuizIndex === 1 ? 'Trung Bình' : 'Khó')}
                         </span>
                        ...
                        <div className="space-y-4 animate-pop-in">
                           <div className={`p-4 rounded-xl border-l-4 ${quizStatus[currentQuizIndex] === 'correct' ? 'bg-green-900/40 border-green-500' : 'bg-slate-800 border-amber-500'}`}>
                             <div className="flex items-center mb-2 font-bold text-sm uppercase tracking-wider">
                                {quizStatus[currentQuizIndex] === 'correct' ? <><span className="text-green-400 mr-2">Chính xác!</span> <span className="text-gold">+{currentQuizIndex === 0 ? 10 : currentQuizIndex === 1 ? 20 : 30} XP</span></> : <span className="text-amber-400 flex items-center"><Lightbulb className="w-4 h-4 mr-1"/> Giải thích chi tiết</span>}
                             </div>
                             <p className="text-slate-200 font-serif leading-relaxed">{currentQuiz.explanation}</p>
                           </div>
                           
                           <div className="flex justify-between mt-4">
                              <button disabled={currentQuizIndex === 0} onClick={() => setCurrentQuizIndex(prev => prev - 1)} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 disabled:opacity-30 transition flex items-center"><ChevronLeft className="w-4 h-4 mr-1" /> Trước</button>
                              <button disabled={currentQuizIndex === quizzes.length - 1} onClick={() => setCurrentQuizIndex(prev => prev + 1)} className={`px-4 py-2 rounded-lg transition flex items-center font-bold ${quizStatus[currentQuizIndex] === 'correct' ? 'bg-green-600 hover:bg-green-500' : 'bg-white/10 hover:bg-white/20'}`}>{currentQuizIndex < quizzes.length - 1 ? "Câu tiếp theo" : "Hoàn thành"} <ChevronRight className="w-4 h-4 ml-1" /></button>
                           </div>
                        </div>
                      )}
                    </div>
          ...
          {result.suggestions && result.suggestions.length > 0 && (
            <div className="mt-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-[#004488] uppercase flex items-center"><Map className="w-6 h-6 mr-2 text-amber-500" /> Hành trình tiếp theo</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {result.suggestions.map((word, idx) => (
                  <button key={idx} onClick={() => handleSearch(null, word)} className="bg-white p-4 rounded-xl shadow-md border-2 border-slate-100 hover:border-amber-400 hover:shadow-lg transition group text-left relative overflow-hidden">
                    ...
                    <p className="text-xs text-slate-400 font-bold uppercase mb-1 relative z-10">Gợi ý {idx + 1}</p>
    ...
    <div className="min-h-screen bg-slate-50 font-serif text-slate-800 relative" ref={topRef}>
      ...
      {notification && (
        <div className="fixed top-24 right-4 z-[70] animate-float-up bg-white border-l-4 border-green-500 shadow-xl rounded-lg p-4 flex items-center pr-8">
           <div className={`p-1 rounded-full mr-3 ${notification.type === 'success' ? 'bg-green-100 text-green-600' : notification.type === 'error' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>{notification.type === 'success' ? <CheckCircle className="w-5 h-5"/> : notification.type === 'error' ? <XCircle className="w-5 h-5"/> : <Zap className="w-5 h-5"/>}</div>
      ...
      {showDailyBonus && (
        <div className="fixed inset-0 z-[90] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowDailyBonus(false)}>
           <div className="bg-white rounded-3xl p-8 text-center max-w-sm w-full animate-pop-in relative overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <h2 className="text-2xl font-bold text-[#004488] mb-2 uppercase">Chào ngày mới!</h2>
              <p className="text-lg text-slate-600 mb-6">Bạn nhận được phần quà điểm danh.</p>
              ...
              <button onClick={claimDailyBonus} className="w-full py-3 bg-[#004488] text-white font-bold rounded-xl hover:bg-blue-700 transition">Nhận thưởng ngay</button>
           </div>
        </div>
      )}
      ...
      {showBadgeDetails && (
        <div className="fixed inset-0 z-[85] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowBadgeDetails(null)}>
           <div className="bg-white rounded-3xl p-8 text-center max-w-sm w-full animate-pop-in" onClick={e => e.stopPropagation()}>
              ...
              <h2 className="text-xl font-bold text-[#004488] mb-2">Huy Hiệu Level {showBadgeDetails}</h2>
              <p className="text-slate-600 mb-6">
                {level >= showBadgeDetails ? "Bạn đã mở khóa huy hiệu này. Xuất sắc!" : `Cần đạt Level ${showBadgeDetails} để mở khóa. Hãy tiếp tục học nhé!`}
              </p>
              <button onClick={() => setShowBadgeDetails(null)} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-600 font-bold transition">Đóng</button>
           </div>
        </div>
      )}
      ...
      {showLevelUp && (
        <div className="fixed inset-0 z-[80] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowLevelUp(false)}>
           <div className="bg-white rounded-3xl p-8 text-center max-w-md w-full animate-pop-in relative overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <h2 className="text-3xl font-bold text-[#004488] mb-2 uppercase">Thăng Cấp!</h2>
              <p className="text-xl font-bold text-slate-600 mb-6">Chào mừng đến với Level {level}</p>
              <div className="bg-blue-50 p-4 rounded-xl mb-6 border border-blue-100"><p className="text-sm text-slate-500 uppercase font-bold mb-1">Danh hiệu mới</p><p className="text-2xl font-bold text-amber-500">{getCurrentRank().title}</p></div>
              <button onClick={() => setShowLevelUp(false)} className="w-full py-3 bg-[#004488] text-white font-bold rounded-xl hover:bg-blue-700 transition">Tiếp tục chinh phục</button>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/95 flex items-center justify-center p-4">
           <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden relative min-h-[400px] flex flex-col animate-pop-in">
              <div className="p-4 bg-[#004488] text-white flex justify-between items-center">
                 <h3 className="font-bold flex items-center"><Timer className="w-5 h-5 mr-2" /> Thử Thách Tốc Độ</h3>
                 ...
                 {!speedReviewData.isFinished ? (
                    <>
                       <div className="mb-8 text-center">
                          <p className="text-slate-400 text-sm uppercase font-bold mb-2">Câu {speedReviewData.currentIndex + 1}/{speedReviewData.questions.length}</p>
                    ...
                    <div className="text-center animate-pop-in">
                       ...
                       <h2 className="text-3xl font-bold text-[#004488] mb-2">Hoàn Thành!</h2>
                       <p className="text-xl text-slate-600 mb-6">Bạn trả lời đúng <span className="font-bold text-green-600">{speedReviewData.score}/{speedReviewData.questions.length}</span> câu.</p>
                       <div className="bg-orange-50 p-4 rounded-xl mb-6 border border-orange-200">
                          <p className="text-sm text-slate-500 uppercase font-bold mb-1">Phần thưởng</p>
                          <p className="text-3xl font-bold text-orange-600">+{speedReviewData.score * XP_REWARDS.SPEED_REVIEW_CORRECT} XP</p>
                       </div>
                       <button onClick={() => { addXP(speedReviewData.score * XP_REWARDS.SPEED_REVIEW_CORRECT); setSpeedReviewMode(false); }} className="px-8 py-3 bg-[#004488] text-white font-bold rounded-xl hover:bg-blue-700 transition">Nhận thưởng & Đóng</button>
                    </div>
                 )}
              </div>
           </div>
            ...
            <div className="flex-1 p-8 perspective-1000 flex items-center justify-center cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
              <div className={`relative w-full h-64 transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                <div className={`absolute inset-0 bg-gradient-to-br from-[#004488] to-[#003366] rounded-2xl flex flex-col items-center justify-center text-white backface-hidden p-6 shadow-xl ${isFlipped ? 'hidden' : 'flex'}`}>
                  ...
                  <p className="mt-8 text-sm text-slate-300 uppercase tracking-widest animate-pulse">Nhấn để lật thẻ</p>
                </div>
      ...
      <header className="sticky top-0 z-50 bg-white border-b-4 border-gold shadow-md">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-[#004488] uppercase tracking-wide leading-none drop-shadow-sm">SAT Thủ Khoa</h1>
              <span className="text-xs md:text-sm text-slate-500 font-medium tracking-widest uppercase">Đức An • Education</span>
            </div>
          </div>
          <div className="flex items-center space-x-2 md:space-x-4">
             <div className="hidden md:flex items-center px-3 py-1 bg-orange-50 text-orange-600 rounded-full border border-orange-200" title="Chuỗi ngày học liên tiếp">
                <Flame className={`w-5 h-5 mr-1 ${streak > 0 ? 'fill-orange-500' : 'text-slate-300'}`} />
                <span className="font-bold">{streak} Ngày</span>
             </div>

            <nav className="hidden md:block">
                <a href="https://SATFelis.Izteach.vn" target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 bg-[#004488] text-white px-5 py-2 rounded-full font-bold shadow-md hover:bg-amber-500 hover:text-[#004488] transition-all transform hover:-translate-y-0.5"><span>Website Chính Thức</span><ExternalLink className="w-4 h-4" /></a>
            </nav>
          </div>
        </div>
      </header>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 shadow-[0_-5px_10px_rgba(0,0,0,0.05)] z-40 flex justify-around items-center px-2 py-2 pb-safe">
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-wide">Học tập</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-wide">Thành tích</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-wide">Kho từ</span>
        </button>
      </div>
